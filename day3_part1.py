...
def run():
    ...
    for instruction in wire_a_raw.split(","):
        path_a.extend(construct_path(path_a[-1], instruction))

    path_b = [origin]
    for instruction in wire_b_raw.split(","):
        path_b.extend(construct_path(path_b[-1], instruction))

    intersections = set(path_a).intersection(set(path_b))
    intersection_distances = sorted([distance(p) for p in intersections])
    min_distance = intersection_distances[1]
