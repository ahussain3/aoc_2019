...
def mhtn_distance(point: Point):
    return abs(point.x) + abs(point.y)

def path_distance_for_wire(wire: List[Point], point: Point):
    distance = 0
    for p in wire:
        if p == point:
            return distance
        ...
        assert False, f"The wire does not pass through point: {point}"

def path_distance(wire_a: List[Point], wire_b: List[Point], point: Point):
    dist_a = path_distance_for_wire(wire_a, point)
    dist_b = path_distance_for_wire(wire_b, point)
    return dist_a + dist_b

def construct_path(start: Point, instruction: str):
    direction = instruction[0]
    distance = int(instruction[1:])

    if direction == "U":
        return [Point(start.x, start.y + i) for i in range(1, distance + 1)]
    elif direction == "D":
        return [Point(start.x, start.y - i) for i in range(1, distance + 1)]
    elif direction == "L":
        return [Point(start.x - i, start.y) for i in range(1, distance + 1)]
    elif direction == "R":
        return [Point(start.x + i, start.y) for i in range(1, distance + 1)]
    else:
        assert False, f"not a direction: {direction}"

def run():
    wire_a = [origin]
    for instruction in wire_a_raw.split(","):
        wire_a.extend(construct_path(wire_a[-1], instruction))

    wire_b = [origin]
    for instruction in wire_b_raw.split(","):
        wire_b.extend(construct_path(wire_b[-1], instruction))

    intersections = set(wire_a).intersection(set(wire_b))
    intersection_distances = [path_distance(wire_a, wire_b, p) for p in intersections]
    min_distance = sorted(intersection_distances)[1]

    # print(f"Path A: {wire_a}")
    # print(f"Path B: {wire_b}")
    print(f"intersections: {intersections}")
    print(f"distances: {intersection_distances}")
